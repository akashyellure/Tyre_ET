    ...
    getJobDetailsTest : function(component, event, helper,Nextlineitem,nextWorkTracker,recordId) {
        ...
        action.setCallback(this, function(response) {
            if (response.getState() == "SUCCESS") { 
				...
				if(component.get("v.recordId")== recordId && respWrapper.jobLineItem.Sequence__c==2){
                        
                        console.log(component.get("v.jobCardIdsToProcess") )
                        
                        var action = component.get('c.getCuringBatchDetailsFromId');            
                        action.setParams({
                            JCIds : component.get("v.jobCardIdsToProcess")            
                        });
                        action.setCallback(this, function(response) {
                            ...
                            if (state === "SUCCESS") {  
                               let data = response.getReturnValue();
                                //alert('JobcardCuring: '+JSON.stringify(data));
                                //alert('JobcardCuring: '+ data);
                               component.set("v.jobCardCuringValveDetails",data);
